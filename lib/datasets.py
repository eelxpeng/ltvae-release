...
class MNISTBinarized(data.Dataset):
    """`MNIST <http://yann.lecun.com/exdb/mnist/>`_ Dataset.
    Args:
        root (string): Root directory of dataset where ``processed/training.pt``
            and  ``processed/test.pt`` exist.
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            ...
            img, target = torch.Tensor(data.shape).uniform_(0,1).le_(data).type(data.dtype), self.train_labels[index]
        else:
            data = self.test_data[index]
            img, target = torch.Tensor(data.shape).uniform_(0,1).le_(data).type(data.dtype), self.test_labels[index]
