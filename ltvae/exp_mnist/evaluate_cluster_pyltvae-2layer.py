...
ltvae.updateLatentTree(args.ltmodel, varNames, setCovariances=False)
...
for var in ltvae.latentTree.latObj._model.getInternalVariables():
    print(var.name)

facets = [0, 1]
latent = latents[facets[1]]
assigments = np.argmax(latent, axis=1)
cond_latent = latents[facets[0]]
num, cluster = latent.shape
